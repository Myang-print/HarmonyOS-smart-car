...
根 `BUILD.gn` 的 `features` 决定实际编入固件的应用。通常一次只启用一个会占用相同 GPIO/UART 的业务示例。当前文件同时启用了 `2.1_TableGuard` 和 `12.0_UART_Correspondence`，两者都会初始化 UART2 并发送电机帧；正式构建前必须按实验目标只保留一个，避免两个任务竞争小车控制权。
