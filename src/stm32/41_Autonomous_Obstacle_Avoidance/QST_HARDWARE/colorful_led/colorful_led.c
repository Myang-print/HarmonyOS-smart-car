...
/**************************************************************************
函数功能：前进状态蓝白呼吸光环推进一帧
说明    ：主循环每20ms调用一次，每5帧推进相位，完整周期约3.2秒
**************************************************************************/
static void LED_State_Forward_Breath_Step(void) {
...
void LED_State_Stop_Step(void) {
  u8 position;

  for (position = 0; position < CAR_RING_LED_COUNT; position++)
    LED_Set_Ring_Pixel(position, 48, 12, 0);
  LED_Show_Frame();
}

void LED_State_LinkFault_Step(void) {
  static u8 lit = 0;
  u8 position;
  ...
  for (position = 0; position < CAR_RING_LED_COUNT; position++) {
    if (lit)
      LED_Set_Ring_Pixel(position, 128, 0, 0);
    else
      LED_Set_Ring_Pixel(position, 0, 0, 0);
  }
  LED_Show_Frame();
}

void LED_State_ProtocolFault_Step(void) {
  static u8 lit = 0;
  u8 position;
  ...
  for (position = 0; position < CAR_RING_LED_COUNT; position++) {
    if (lit)
      LED_Set_Ring_Pixel(position, 96, 0, 96);
    else
      LED_Set_Ring_Pixel(position, 0, 0, 0);
  }
  LED_Show_Frame();
