...
static s16 target_left_speed;
static s16 target_right_speed;
static s16 current_left_speed;
static s16 current_right_speed;
...
void Control_SetWheels(s16 left_speed, s16 right_speed) {
  current_left_speed = ClampSpeed(left_speed);
  current_right_speed = ClampSpeed(right_speed);
  target_left_speed = current_left_speed;
  target_right_speed = current_right_speed;
  SetLeftMotor(current_left_speed);
  SetRightMotor(current_right_speed);
}

void Control_SetTargetWheels(s16 left_speed, s16 right_speed) {
  target_left_speed = ClampSpeed(left_speed);
  target_right_speed = ClampSpeed(right_speed);
}
...
static s16 RampSpeed(s16 current, s16 target) {
  if (current < target) {
    current += CAR_SPEED_RAMP_STEP;
...
void Control_Update(void) {
  current_left_speed = RampSpeed(current_left_speed, target_left_speed);
  current_right_speed = RampSpeed(current_right_speed, target_right_speed);
  SetLeftMotor(current_left_speed);
  SetRightMotor(current_right_speed);
}
...
u8 Control_ExecuteCommand(u8 command) {
  ...
  case '1':
    Control_SetTargetWheels(CAR_FORWARD_SPEED, 0);
    break;
  case '2':
    Control_SetTargetWheels(-CAR_FORWARD_SPEED, 0);
    break;
  case '3':
    Control_SetTargetWheels(0, CAR_FORWARD_SPEED);
    break;
  case '4':
    Control_SetTargetWheels(0, -CAR_FORWARD_SPEED);
    break;
